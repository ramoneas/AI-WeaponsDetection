...
from src.azure_config import azure_object_detection
...
def db_insert_weapon_spc(db, mycursor, pistols, revolvers, machine_gun, sub_machine_gun, grenades):
    weapon_sql = (
        "INSERT INTO weapon_loadout (pistols, revolvers, machine_gun, submachine_gun, grenades) VALUES (%s, %s, %s, %s, %s)")
...
def db_insert_main_data(db, mycursor, last_loadout_id, official, location, transportation, institution):
    ...
    data_val = (official, location, last_loadout_id,
                transportation, institution, datetime.now())
